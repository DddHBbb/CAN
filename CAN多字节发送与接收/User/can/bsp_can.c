...
/**
  ...
  * @retval None
  */
void Init_RxMes(CanRxMsg *RxMessage,uint8_t *buff)
{
  uint8_t ubCounter = 0;
...
//************************************************************************************************************************************************
//函数名：void CAN_SendDataProces(CanTxMsg *TxMessage,u8* data,u8 Num)
//功 能：CAN发送数据前期处理  发送第一步
//参 数：data数据指针；Num数据长度
//返回值：无
//************************************************************************************************************************************************
uint8_t Close_SendMes=1;
uint8_t Record_TxDataLen=0;
uint8_t One_Send=0; //再次发送需置0

int CAN_SendDataProces(CanTxMsg *TxMessage,u8* data,u8 Num)
{
 u8 i;
 uint8_t *s=NULL;
 static uint8_t Count,Left,onetime = 1;
	...
	if((Close_SendMes == 1) && (One_Send != 1))
	{
		if((Count--) > 0)
		{
			 for(i=0;i<8;i++)
			 {
				TxMessage->Data[i]=data[Record_TxDataLen++];
			 }
			 ...
			 return 0;  //每次只发送一次，用return 跳出
	  }
		if(Count == 0xff) Count = 0; //放置溢出
		if((Left < 8)&&(Count == 0)) //发送剩下的，不满8个字节的
		 {
			 for(i=0;i<Left;i++)
			 {
				TxMessage->Data[i]=data[Record_TxDataLen++];
		 ...
		 if(Num == Record_TxDataLen) 
		 {
			 Record_TxDataLen=0; //放置记录数组下标的变量溢出，它的最大值等于数组长度
			 One_Send =1;   //到此处发送完毕，关闭发送函数。
		 }
...
void CAN_REVDataProces(CanRxMsg *RxMessage,u8* data)
{
 u8 i;
 static u8 Record_RxDataLen;//记录接收数组下标，保证挨着上次接收的位置继续接收

 for(i=0;i<8;i++)
 {
	 data[Record_RxDataLen++] = RxMessage->Data[i];
 }

 Close_SendMes = 1;  //关闭发送函数，保证是接收完再发
